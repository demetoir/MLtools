...
class DVAE(AbstractModel):
    ...
    def load_main_tensor_graph(self):
        ...
        self.mean, self.std = self.encoder(self.Xs_noised)
        self.zs_gen = tf.add(self.mean, self.std * tf.random_normal(self.mean.shape, 0, 1, dtype=tf.float32),
                             name='zs_gen')
    ...
    def get_noise(self):
        return np.random.normal(-1., 1., self.noise_shape)
