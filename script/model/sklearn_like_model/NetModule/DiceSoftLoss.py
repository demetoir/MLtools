...
class DiceSoftLoss(BaseLossModule):
    ...
    def build(self):
        ...
        self.intersection = tf.reduce_sum(true * proba, name='intersection')
        # self.union = tf.reduce_sum((true * true) + (proba * proba), name='union')
        self.union = tf.reduce_sum(true + proba, name='union')
